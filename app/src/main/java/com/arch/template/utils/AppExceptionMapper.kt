...
object AppExceptionMapper {
    fun init(context: Context) {
        ExceptionMappers
            ...
            .condition(
                condition = {
                    (it is AppError) && it.appErrorType == AppErrorType.TitleEmpty
                },
                mapper = {
                    context.getString(R.string.app_error_empty_title)
                }
            )
            .condition(
                condition = {
                    (it is AppError) && it.appErrorType == AppErrorType.SubTitleEmpty
                },
                mapper = {
                    context.getString(R.string.app_error_empty_sub_title)
                }
            )
